...
def main():
    ...
    browser = cef.CreateBrowserSync(url="file:///autoDownload/mainlayout.html",
...
class External(object):
    ...
    def listFiles(self):
        ...
        for driveFile in file_list:
            ...
            while (fileName in self.ftid.keys()):
                tmp = fileName.split(".")
                fileName = "1_" + fileName
            self.ftid[fileName] = driveFile["id"]
            js_print(self.browser,"Python", "Drive file detected", fileName)
        print(self.ftid)
    ...
    def set_javascript_bindings(self):
        ...
        bindings.SetProperty("initOnce", self.initOnce)
        bindings.SetProperty("driveID", self.user_settings.getDriveFolderID())
    ...
    def downloadFile(self,fileName):
        ...
        f = drive.CreateFile({'id': self.ftid[fileName]})
        
        if self.user_settings.isMobile:
            f.GetContentFile(fileName)
            getFileToDroid(self.user_settings.recordingPath,fileName)
            f.SetContentFile(fileName) # I dont think this line is needed but I can't test it right now so ehh
    
        else:
            filePath=self.user_settings.recordingPath
            f.GetContentFile(p.join(filePath, fileName)) 
        '''
        try:
                f.Upload() 
